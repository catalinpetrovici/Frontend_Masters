...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  let bipEvent = null; // before install prompt
  window.addEventListener('beforeinstallprompt', (event) => {
    event.preventDefault();
    bipEvent = event; // before install prompt
  });
  ...
  document.querySelector('#btnInstall').addEventListener('click', (event) => {
    if (bipEvent) {
      ...
      // incompatible browser, your PWA is not passing the criteria, the user has already install the PWA
